...
const Button = ({
  ...
  onClick,
  isQuestionNumber
}) => <button onClick={onClick} disabled={disabled} className={`${styles.buttonQuestion} ${styles.buttonQuestionNumber}`}>{children}</button>;
...
export class QuestionsButtons extends React.Component {
  render() {
    ...
    const upperLimit = Math.min(currentQuestionId + LIMIT, totalNumberOfQuestion - 1);
    ...
    const lowerButtons = [...Array(currentQuestionId - lowerLimit)].map((_, index) => {
      ...
      return <Button onClick={() => onQuestionChangeHandler(id)}>{id + 1}</Button>;
    });
